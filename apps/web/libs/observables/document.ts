...
type PageState = { state: "hidden" | "active" | "passive" };

export const pageVisibility$ = new Subject<PageState>();

export const pageUnload$ = !isSSR()
...
function observePageLifeCycle() {
  ...
  const logStateChange = (nextState: any) => {
  ...
  return () => {
    for (let fn of Array.from(handles.values())) {
      // unsubscribe
      fn();
    }
    handles.clear();
